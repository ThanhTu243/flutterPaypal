...
class PaypalPayment extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: WebView(
        initialUrl:
            'http://192.168.8.1:3000/createpaypalpayment?amount=$amount&currency=$currency',
        ...
        navigationDelegate: (NavigationRequest request) async {
          if (request.url.contains('http://return_url/?status=success')) {
            ...
            Navigator.pop(context);// đẩy trang thành công vào
          }
          if (request.url.contains('http://cancel_url')) {
            Navigator.pop(context); // đẩy trang thất bại vào
          }
